...
# Batch operation response containing user configuration results with potential errors.
public type BatchResponsePublicAssociationDefinitionUserConfigurationWithErrors record {
    ...
    string startedAt;
    # A map of related resource URLs associated with the batch response.
    record {|string...;|} links?;
    # Array of successfully processed user configuration results.
...
# Request object for creating a new association definition between CRM objects.
...
# Collection response containing user configurations without pagination.
public type CollectionResponsePublicAssociationDefinitionUserConfigurationNoPaging record {
    # Array of user configuration objects in the collection.
    PublicAssociationDefinitionUserConfiguration[] results;
};

# Container for batch input of association specifications.
public type BatchInputPublicAssociationSpec record {
    # Array of association specifications to process in the batch request.
    PublicAssociationSpec[] inputs;
};

# Batch response containing association definition configuration update results.
public type BatchResponsePublicAssociationDefinitionConfigurationUpdateResult record {
    ...
    record {|string...;|} links?;
    # Array of configuration update results from the batch operation.
...
# Request object for updating an association definition configuration.
...
# Request object for creating a new association definition configuration.
...
# Request payload for updating an association definition's label and inverse label.
...
# Detailed information about a specific error that occurred during an API operation.
...
# User-defined configuration settings for an association definition.
...
# Response containing results from a batch operation on association definition configurations.
...
# Batch request payload containing multiple association definition configuration updates.
...
# Batch response containing update results and any errors that occurred during processing.
public type BatchResponsePublicAssociationDefinitionConfigurationUpdateResultWithErrors record {
    # The timestamp when the batch operation finished processing.
    string completedAt;
    # The total count of errors encountered during the batch operation.
    int:Signed32 numErrors?;
    # The timestamp when the batch operation was initially requested.
    string requestedAt?;
    # The timestamp when the batch operation began processing.
    string startedAt;
    # Map of related resource URLs associated with the batch response.
    record {|string...;|} links?;
...
# Batch request payload for creating multiple association definition configurations.
