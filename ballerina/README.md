...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses manage marketing, sales, customer service, and content management through an integrated suite of tools designed to drive growth and streamline operations.

The `ballerinax/hubspot.crm.associations.schema` package offers APIs to connect and interact with [HubSpot CRM Associations Schema API](https://developers.hubspot.com/docs/api/crm/associations) endpoints, specifically based on [HubSpot CRM Associations Schema API v4](https://developers.hubspot.com/docs/api/crm/associations/v4).
## Setup guide
...
2. If you intend to use private apps for API access, note that creating private apps requires a HubSpot account with Super Admin permissions. Additionally, certain API features may require a Professional or Enterprise plan subscription depending on the scope of access needed.
...
5. On the Basic Info tab, enter your app's name and description.

6. Navigate to the **Scopes** tab and select the required scopes for CRM associations schema access (such as `crm.schemas.contacts.read`, `crm.schemas.contacts.write`, or other relevant schema scopes).
...
2. [Association analytics report](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations.schema/tree/main/examples/association_analytics_report) - Illustrates how to generate analytics reports based on CRM association schema data.
3. [Automated configuration update](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations.schema/tree/main/examples/automated_configuration_update) - Shows how to automate the updating of association schema configurations in HubSpot CRM.