...
6. Navigate to the **Scopes** tab and select the required scopes for the CRM Associations Schema API (such as `crm.schemas.custom.read` and `crm.schemas.custom.write`).

7. Click **Create app** in the top right corner.

8. Review the information in the dialog box, then click **Continue creating**.

9. After the app is created, click **Show token** to reveal your access token.
...
public function main() returns error? {
    hscrmschema:CollectionResponsePublicAssociationDefinitionUserConfigurationNoPaging response = check hscrmschemaClient->/definitions/configurations/all.get();
}
```
