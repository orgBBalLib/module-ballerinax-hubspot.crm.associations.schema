...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses grow by connecting marketing, sales, content management, and customer service teams on a single unified platform.
...
2. If you intend to use private apps for API access, note that while HubSpot offers free tools, certain API features and higher rate limits may require a Professional or Enterprise plan subscription.
...
8. Once the private app is created, navigate to the Access token section and click Show token, then copy the generated access token.
...
public function main() returns error? {
    hsassocschema:PublicAssociationDefinitionCreateRequest newLabel = {
        name: "partner_relationship",
        label: "Partner",
        inverseLabel: "Partner Of"
    };

    hsassocschema:CollectionResponseAssociationSpecWithLabelNoPaging response = check hsassocschemaClient->/["contacts"]/["companies"]/labels.post(newLabel);
}
```
...
3. [Automated configuration update](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations.schema/tree/main/examples/automated_configuration_update) - Shows how to automate the updating of association schema configurations.
