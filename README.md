...
[HubSpot](https://www.hubspot.com/) is a cloud-based customer relationship management (CRM) platform that helps businesses manage their marketing, sales, customer service, and content management through integrated tools and automation.
...
2. If you intend to use private apps for API access, note that some API features may require a Professional or Enterprise plan subscription, depending on the specific endpoints and scopes you need to access.
...
6. Navigate to the Scopes tab and select the required scopes for CRM associations (such as `crm.schemas.associations.read` and `crm.schemas.associations.write`).

7. Click Create app in the top right corner, then click Continue creating to confirm.

8. After the app is created, navigate to the Auth tab and copy the access token displayed.
...
2. [Association analytics report](https://github.com/ballerina-platform/module-ballerinax-hubspot.crm.associations.schema/tree/main/examples/association_analytics_report) - Illustrates how to generate analytical reports based on CRM association schema data.
